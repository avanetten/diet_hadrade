...
def plot_graph_pix(G, im=None, bbox=None, fig_height=6, fig_width=None, margin=0.02,
    ...
    node_Xs = [float(x) for n, x in G.nodes(data='x_pix') if n > 0]
    node_Ys = [float(y) for n, y in G.nodes(data='y_pix') if n > 0]
   ...
   # get north, south, east, west values either from bbox parameter or from the
    ...
    if bbox is None:
        ...
        # make sure axes aren't inverted
        # if west > 
...
def plot_graph_route_pix(G, route, im=None, bbox=None, fig_height=6, fig_width=None,
    ...
    G : networkx multidigraph
    route : list
        the route as a list of nodes, (can also be multple routes)
    bbox : tuple
        bounding box as north,south,east,west - if None will calculate from
        spatial extents of data. if passing a bbox, you probably also want to
        pass margin=0 to constrain it.
    ...
    # check if we have multiple routes, or just one
    if type(route[0]) == list:
        routes = route
    else:
        # wrap the single route
        routes = [route]
